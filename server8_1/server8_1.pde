...
////////////////////////////////////////////
#include <avr/pgmspace.h>
prog_char type_temp[] PROGMEM = "sensor.temperature";   // "String 0" etc are strings to store - change to suit.
prog_char type_hum[] PROGMEM = "sensor.humidity";
prog_char type_light[] PROGMEM = "sensor.light";
prog_char type_volt[] PROGMEM = "sensor.voltage";
prog_char type_elec[] PROGMEM = "sensor.electrcity";
...
// buffer 
byte Ethernet::buffer[350];   // a very small tcp/ip buffer is enough here
...
void loop () {
  ...
  if (rf12_recvDone() && rf12_crc == 0 )
  {
      ...
      Serial.print("SENDID:");Serial.println(SenderID);Serial.println("     ");
      //http://www.22balmoralroad.net/wordpress/wp-content/uploads/homeBase.pde
    ...
    if( rf12_len == sizeof emontx){
       ...
       sprintf(line_buf,"{\'ct1\':%d,\'ct2\':%d,\'mac\':\'%s\'}", emontx.ct1,emontx.ct2,"mac");  
       //sprintf(line_buf,"[{\'moteId\':\'%s\',\'type\':\'http://webinos.org/api/motes.temperature\',\'data\':[%d]},{\'moteId\':\'%s\',\'type\':\'http://webinos.org/api/motes.humidity\',\'data\':[%d]}]",temid,emonen.temperature,humid,emonen.humidity); 
    ...
    if(rf12_len == sizeof emonen)
    {
       ...
       if(strcmp(emonen.mac,"000000000000")==0){
         Serial.println(" mac not identified");
         goto NO_MAC;
       }
...
void storeDevice(char *id,byte *type){
  int length = 0;
  ...
  {      
    ...
    if(created==true)
    {
      ...
      {        
        ...
        Serial.println(type);
        bfill.emit_p(PSTR("{\"sensorId\":\"$S\""), tempid); // start address from 0
        //Serial.println(id);
      ...
      {
        bfill.emit_p(PSTR("{\"actuatorId\":\"$S\""), id);   // start address from 0
...
{
  //Serial.print("info   ");Serial.println(id);    Serial.println(strlen(id));  
     ...
     // Serial.print("==========================found:    ");Serial.print(id);Serial.print("   compares:  ");Serial.println(line_buf);
      ...
      if(found)
      {
         ...
         if(deviceGroup ==0)
          ...
          bfill.emit_p(PSTR("{\"actuatorId\":\"$S\""), id);        
        ...
        bfill.emit_p(PSTR("\"actuatorType\":\"$S\""), line_buf);
